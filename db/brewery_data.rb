def get_brewery_data
  brewery_data = {

    "denizens" =>{
      name: 'Denizens Brewing Co.',
      photo_url: 'http://www.clotureclub.com/wp-content/uploads/2015/07/Beer-Garden.jpg',
      logo_img: '',
      address: '1115 East-West Highway, Silver Spring, MD 20910'
    },
    "dc_brau" =>{
      name: 'DC Brau',
      photo_url: 'https://rhodeislandne.files.wordpress.com/2011/12/photo-25.jpg',
      logo_img: '',
      address: '3178 Bladensburg Road Northeast, Washington, DC 20018'
    },
    "three_stars" =>{
      name: '3 Stars Brewing Company',
      photo_url: ' ',
      logo_img: '',
      address: '6400 Chillum Place Northwest, Washington, DC 20011'
    },
    "hellbender" =>{
      name: 'Hellbender Brewing Company',
      photo_url: ' ',
      logo_img: ' ',
      address: '5788 2nd Street NE, Washington, DC 20011'
    },
    "atlas" =>{
      name: 'DC Brau',
      photo_url: ' ',
      logo_img: '',
      address: '2052 West Virginia Avenue Northeast #102, Washington, DC 20002'
    },
    "bardo" =>{
      name: 'Bardo Brewpub',
      photo_url: ' ',
      logo_img: '',
      address: '1200 Bladensburg Road Northeast, Washington, DC 20002'
    },
    "right_proper_shaw" =>{
      name: 'Right Proper Brewpub & Kitchen',
      photo_url: 'http://static1.squarespace.com/static/55b03c0fe4b01db83f825289/t/575317891bbee04c3d4fe25b/1465063309608/shaw_exterior1.JPG?format=1500w',
      logo_img: '',
      address: '624 T St NW, Washington, DC 20001'
    },
    "port_city" =>{
      name: 'Port City Brewing Company',
      photo_url: 'https://www.kimptonhotels.com/blog/wp-content/uploads/2014/10/Port-City.jpg',
      logo_img: '',
      address: '3950 Wheeler Ave, Alexandria, VA 22304'
    },
    "bluejacket" =>{
      name: 'Bluejacket',
      photo_url: 'https://brewstravelers365.files.wordpress.com/2014/03/dsc05243.jpg',
      logo_img: '',
      address: '300 Tingey St SE, Washington, DC 20003'
    },
    "right_proper_brookland" => {
      name: 'Right Proper Brookland',
      photo_url: 'http://static1.squarespace.com/static/55b03c0fe4b01db83f825289/t/569ed676b20943c52005a460/1453250167438/brookland_corner_exterior.JPG?format=1500w',
      logo_img: '',
      address: '920 Girard St NE, Washington, DC 20017'
    }
  }

  return brewery_data
end
